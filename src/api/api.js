...
const setStorageData = (key, data) => {
  return new Promise((resolve, reject) => {
    ...
    chrome.storage.local.set(storageObj, () => {
      if (chrome.runtime.lastError) {
        console.error(`Storage set error for ${key}:`, chrome.runtime.lastError);
        reject(chrome.runtime.lastError);
      } else {
...
const filterByUrlKey = (data, urlKey) => {
  return Array.isArray(data) ? data.filter(item => item.URL_KEY === urlKey) : [];
};
...
let Api = {
  ...
  getBackupData: async (email) => {
    try {
      const [sites, highlights, options, categories, categoryRelations, onetabs] = await Promise.all([
      ...
      return {
        sites: [], highlights: [], options: [],
        categories: [], categoryRelation: [], onetabs: []
      };
    }
  },
  ...
  getBackupSites: async (param) => {
    ...
    return filterByEmail(sites, param.EMAIL).filter(site => site.FL_DELETE !== 'Y');
  },

  getBackupHighlights: async (param) => {
    const items = await getStorageData(STORAGE_KEYS.ITEMS);
    return filterByEmail(items, param.EMAIL).filter(item => item.FL_DELETE !== 'Y');
  },
  ...
  getBackupCategorys: async (param) => {
    ...
    return filterByEmail(categories, param.EMAIL).filter(cat => cat.TYPE === 'CUSTOM');
  },
  ...
  getNoCategoryCount: async (param) => {
    ...
    const noCategoryItems = userItems.filter(item => !item.CATEGORY_ID || item.CATEGORY_ID === -1);
  ...
  getSite: async (param) => {
    ...
    return filterByUrlKey(userSites, param.URL_KEY).filter(site => site.FL_DELETE !== 'Y');
  },
  ...
  updateItem: async (param) => {
    const items = await getStorageData(STORAGE_KEYS.ITEMS);
    const itemIndex = items.findIndex(item => item.IDX === param.IDX && item.EMAIL === param.EMAIL);
  ...
  deleteSiteInCategory: async (param) => {
    ...
    const filteredRelations = relations.filter(rel =>
      !(rel.URL_KEY === param.URL_KEY && rel.EMAIL === param.EMAIL)
    );
    return await setStorageData(STORAGE_KEYS.CATEGORY_RELATIONS, filteredRelations);
  },
  ...
  updateScrapSite: async (param) => {
    ...
    param.READERMODE_CONTENTS = Common.replaceSpecialWord(param.READERMODE_CONTENTS);
  ...
  deleteCategory: async (param) => {
    ...
    const filteredCategories = categories.filter(cat =>
      !(cat.IDX === param.IDX && cat.EMAIL === param.EMAIL)
    );
  ...
  deleteCategoryRelationParent: async (param) => {
    const relations = await getStorageData(STORAGE_KEYS.CATEGORY_RELATIONS);
    const filteredRelations = relations.filter(rel => rel.PARENT_IDX !== param.PARENT_IDX);
    return await setStorageData(STORAGE_KEYS.CATEGORY_RELATIONS, filteredRelations);
  },
  ...
  updateLostCategoryItem: async (param) => {
    ...
    const updatedCategories = categories.map(cat => {
      if (cat.PARENT_ID && cat.PARENT_ID.includes(param.categoryId)) {
        return { ...cat, PARENT_ID: cat.PARENT_ID.replace(param.categoryId, '-1') };
      }
      return cat;
    });
  ...
  getCategoryMaxId: async () => {
    const categories = await getStorageData(STORAGE_KEYS.CATEGORIES);
    const maxId = categories.length > 0 ? Math.max(...categories.map(cat => cat.IDX || 0)) : 0;
    return [{ MAXID: maxId }];
  },
