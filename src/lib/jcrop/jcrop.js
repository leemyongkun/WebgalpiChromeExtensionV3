...
let JCROP = {
  ...
  init: (target, options, callback) => {
    ...
    _this.each(function() {
      ...
      if ($(_this).data("Jcrop")) {
        // The API can be requested this way (undocumented)
        if (options === "api") return $(_this).data("Jcrop");
        // Otherwise, we just reset the options...
        else
          $(_this)
            .data("Jcrop")
      ...
      // If we haven't been attached, preload and attach
        ...
        if (_this.tagName == "IMG")
          $.Jcrop.Loader(_this, function() {
            $(_this).css({ display: "block", visibility: "hidden" });
            api = $.Jcrop(_this, options);
            if ($.isFunction(callback)) callback.call(api);
          });
        else {
          $(_this).css({ display: "block", visibility: "hidden" });
