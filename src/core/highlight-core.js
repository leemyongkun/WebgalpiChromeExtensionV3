...
var highlightRange = (function () {
    ...
    function highlightRange(rangeObject, highlightClass , highlightTag, highlightId, hlghlightText) {
        ...
        return new Promise(function(res){
            ...
            for (nodeIdx in nodes) {
                let ret = highlightNode(nodes[nodeIdx], highlightClass, highlightTag, highlightId, count);
                if(!ret){
                    //console.log("ret ",ret);
                    highlights.push(ret);
                }
    ...
    function highlightNode(node, highlightClass , highlightTag, highlightId, count) {

        var BR = document.createElement("BR");
        if($(node).closest('style').length != 0) return false;
        if($(node).closest('script').length != 0) return false;
        if($(node).parent().css('display') == 'none' ) return false;
