...
const BackgroundModule = {
  ...
  initApplication: (tabId, currentUrl) => {
    ...
    Api.getMemberInfo().then(memberInfo => {
      ...
      if (memberInfo.EMAIL === "") {
        console.log("❌ No member info, skipping Content Script initialization");
        return false;
      }
      ...
      Api.getInitInfo(initParameter).then(res => {
        ...
        if (res.isRegist) {
          let highlightSize = res.allItems.HIGHLIGHT_LIST == null
            ? 0 
            : res.allItems.HIGHLIGHT_LIST.length;
          console.log("✅ Site registered, setting badge with highlight count:", highlightSize);
        ...
        console.log("📤 Sending application.init message to Content Scripts for tabId:", tabId);
          ...
          chrome.tabs.sendMessage(tabId, {
            ...
            if (chrome.runtime.lastError) {
              if (attempt < maxAttempts) {
                console.warn(`⚠️  Attempt ${attempt} failed, retrying in ${attempt * 500}ms:`, chrome.runtime.lastError.message);
                setTimeout(() => sendMessageWithRetry(attempt + 1, maxAttempts), attempt * 500);
              } else {
                console.error("❌ Could not send application.init to Content Scripts after", maxAttempts, "attempts:", chrome.runtime.lastError);
              }
            } else {
              console.log("✅ Content Scripts initialized successfully for tabId:", tabId, "on attempt", attempt);
            }
          });
        };
...
chrome.tabs.onActivated.addListener((activeInfo) => {
  chrome.tabs.get(activeInfo.tabId, (tab) => {
    ...
    if (tab && tab.url && tab.status === 'complete') {
      ...
      console.log("Tab still loading, skipping initialization for tabId:", tab.id);
    } else {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  switch (msg.type) {
    ...
    case "get.category":
      Api.getCategory(msg.data).then(result => {
    ...
    case "get.option":
      Api.getOptions(msg.data).then(result => {
    ...
    case "get.highlights":
      ...
      Api.getAllItems(highlightParam).then(res => {
