...
export default {
  ...
  methods: {
    ...
    setEvents() {
      ...
      const drawerBorder = el.querySelector ? el.querySelector(".v-navigation-drawer__border") : null;
      
      // Skip event setup if drawerBorder doesn't exist (removed Vuetify components)
      if (!drawerBorder) {
        console.warn("Drawer border element not found - skipping resize events");
        return;
      }
